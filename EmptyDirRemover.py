...
def recursiveClean(toClean: str) -> None:
    ...
    for contained in dirContent:
        ...
        if (os.path.isdir(newPath)) : recursiveClean(newPath)

    # Refresh the content list and the eliminates if empty
    dirContent = os.listdir(toClean)
    if (len(dirContent) == 0) : os.rmdir(toClean)
...
def EmptyDirRemover() -> None:
    ...
    else : log.error("The input given is not a directory")


if __name__ == "__main__": EmptyDirRemover()
