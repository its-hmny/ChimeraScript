...
def loadScriptFromArray(array):
    for string in array:
        ...
        execFunction = lambda command: __import__("os").system(command)
...
def PyHypervisor():
    ...
    log.error("No script scheduled for execution") if len(sys.argv) == 1 else log.success("---> All task completed")
