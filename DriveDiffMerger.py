...
# This are mappings to the root folder in Drive and to the respective folder on my computers
if (platform.system() == "Windows"):
    ...
    blacklist = ["Modelli di Office personalizzati", "WindowsPowerShell",
                 "desktop.ini", "Musica", "Video", "Immagini", "Foto", "Camera Roll", "Saved Pictures"]
...
def tupleIterator(first, second,  valueFirst, valueSecond):
    tmpFirst = [valueFirst(item)
                for item in first if not item in blacklist]
    tmpSecond = [valueSecond(item)
                 for item in second if not item in blacklist]
    conglomerate = []

    # Pair every elements wih its counterpart, if couterpart is not found pair itself with None
    ...
    # The remaining file are paired with None but specularly in relation to above 
...
# Synchronizes a directory in both direction: remote -> local and local -> remote at the same time
def synchDir(remotepath, localpath):
    os.chdir(localpath)
    remote_entries = drivefs.listDir(remotepath)
    local_entries = [entry for entry in os.listdir('.') if not entry in blacklist]

    def getRemoteName(r_file): return r_file.filename
    # Because of fucking Windows "'file.txt'" string representation instead of "file.txt"
    def getLocalName(l_file): return "{}".format(l_file)

    for r_entry, l_entry in tupleIterator(remote_entries, local_entries, getRemoteName, getLocalName):
        l_exist = l_entry is not None
        r_exist = r_entry is not None
        # If the file/dir already exist on local and remote then additional check has to be done
    ...
    # For recursive call returns to previous directory so the caller can start without inconsistencies
    os.chdir('..')
...
def DriveDiffMerger():
    ...
    # From now on we can assume specularity between the remote and the local tree structure
    for entry in drivefs.listDir('root'):
        ...
        try:
            ...
            log.success("{} synchronized with your Google Drive".format(dirToSync[entry.filename]))
        except KeyError:
            log.error("Missing mapping for Google Drive directory {}".format(entry.filename))
